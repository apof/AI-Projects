    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                for x in cornL:
                    if x[0] == nextx and x[1] == nexty:
                        cornL.remove((nextx,nexty))  

                successors.append((((nextx,nexty),cornL),action,1))
...
def cornersHeuristic(state, problem):
    ...
    for r in range(number):
        ...
        unvisitedCorners.remove(nearest)
        State = nearest
...
def foodHeuristic(state, problem):
    ...
    for f in Food:
        distance = mazeDistance(State, f, problem.startingGameState)
        if distance > cost:
            cost = distance
    return cost
